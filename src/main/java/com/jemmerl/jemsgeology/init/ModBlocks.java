...
public class ModBlocks {
    ...
    public static <T extends Block>RegistryObject<T> registerStoneGeoBlock(GeoType geoType) {
        ...
        Supplier<T> blockSupplier = () -> (T) new StoneGeoBlock(getStoneProp(geoType), geoType, OreType.NONE, OreGrade.NONE);
        return registerBlock(name, blockSupplier, ModItemGroups.JEMSGEO_BASE_STONE_GROUP);
    }

    // For ore-bearing stone blocks
    public static <T extends Block>RegistryObject<T> registerStoneOreBlock(GeoType geoType, OreType oreType, OreGrade oreGrade) {
        String blockTypeName = (geoType.getGeoGroup().isRegolith()) ? "_detritus" : "_stone";
        String name = geoType.getName() + blockTypeName + "/" + oreType.getName() + "/" + oreGrade.getName();
        Supplier<T> blockSupplier = () -> (T) new StoneGeoBlock(getStoneProp(geoType), geoType, oreType, oreGrade);
...
//    }
    ...
    public static <T extends Block>RegistryObject<T> registerRegolithGeoBlock(GeoType geoType) {
        ...
        return registerBlock(name, buildRegolithBlock(geoType, OreType.NONE, OreGrade.NONE), ModItemGroups.JEMSGEO_BASE_STONE_GROUP);
    }

    // For ore-bearing detritus blocks
    public static <T extends Block>RegistryObject<T> registerRegolithOreBlock(GeoType geoType, OreType oreType, OreGrade oreGrade) {
        String name = geoType.getName() + "_rego/" + oreType.getName() + "/" + oreGrade.getName();
        return registerBlock(name, buildRegolithBlock(geoType, oreType, oreGrade), ModItemGroups.JEMSGEO_ORE_BLOCK_GROUP);
    }
    ...
    private static <T extends Block>Supplier<T> buildRegolithBlock(GeoType geoType, OreType oreType, OreGrade oreGrade) {
        switch (geoType) {
            case DIRT:
            case COARSE_DIRT:
            case LATERITE:
                return () -> (T) new BaseGeoBlock(DIRT_PROP, geoType, oreType, oreGrade);
            case CLAY:
                return () -> (T) new BaseGeoBlock(CLAY_PROP, geoType, oreType, oreGrade);
            case SAND:
            case RED_SAND:
            case PINK_SAND:
            case BLACK_SAND:
            ...
            case DUNE_SAND:
            case GYPSUM_SAND:
                return () -> (T) new FallingBaseGeoBlock(SAND_PROP, geoType, oreType, oreGrade);
            case GRAVEL:
                return () -> (T) new FallingBaseGeoBlock(GRAVEL_PROP, geoType, oreType, oreGrade);
            default:
    ...
    public static <T extends Block>RegistryObject<T> registerCobblesBlock(GeoType geoType) {
        ...
        return registerBlock(name, blockSupplier, ModItemGroups.JEMSGEO_COBBLE_GROUP, 16);
    }
    ...
    public static <T extends Block>RegistryObject<T> registerBlock(String name, Supplier<T> block, ItemGroup itemGroup) {
        return registerBlock(name, block, itemGroup, 64);
    }

    public static <T extends Block>RegistryObject<T> registerBlock(String name, Supplier<T> block, ItemGroup itemGroup, int stackSize) {
